...
class ProjectManager:
    ...
    def _setup_directories(self):
        ...
        lib_rel = self.config.get('paths', {}).get('library', 'library')
        proj_rel = self.config.get('paths', {}).get('projects', 'projects')
        
        self.library_path = self.root_dir / lib_rel
        self.projects_path = self.root_dir / proj_rel
...
def main():
    ...
    # Инициализация менеджера с текущей директорией
    manager = ProjectManager(os.getcwd())
